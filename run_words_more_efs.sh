...
# Build with CMake
cmake -DFAISS_ENABLE_GPU=OFF -DFAISS_ENABLE_PYTHON=OFF -DBUILD_TESTING=ON -DBUILD_SHARED_LIBS=ON -DCMAKE_BUILD_TYPE=Release -B build_words
make -C build_words -j faiss
make -C build_words utils
make -C build_words test_acorn
