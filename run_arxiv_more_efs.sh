...
# Build with CMake
cmake -DFAISS_ENABLE_GPU=OFF -DFAISS_ENABLE_PYTHON=OFF -DBUILD_TESTING=ON -DBUILD_SHARED_LIBS=ON -DCMAKE_BUILD_TYPE=Release -B build_arxiv
make -C build_arxiv -j faiss
make -C build_arxiv utils
make -C build_arxiv test_acorn
...
##########################################
for i in {1..1}; do
    query_path="../ACORN_data/arxiv/arxiv_query/arxiv_query_${i}"
    ...
    for efs in "${efs_values[@]}"; do
        ...
        for run in {1..5}; do
            ...
            ./build_arxiv/demos/test_acorn $N $gamma $dataset $M $M_beta $efs "${query_path}" "${generate_json}" &>> ${dir}/summary_run${run}.txt
